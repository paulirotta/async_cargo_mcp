...
async fn test_async_clippy_wait_combines_outputs() -> Result<()> {
    ...
    let resp = client.call_tool(CallToolRequestParam { name: "clippy".into(), arguments: Some(object!({"working_directory": project_path, "enable_async_notification": true, "args": ["--", "-W", "clippy::all"]})) }).await?;
