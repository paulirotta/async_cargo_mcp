...
async fn test_async_fmt_wait_combines_outputs() -> Result<()> {
    ...
    let resp = client.call_tool(CallToolRequestParam { name: "fmt".into(), arguments: Some(object!({"working_directory": project_path, "enable_async_notification": true, "check": true})) }).await?;
