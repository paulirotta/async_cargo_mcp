...
async fn test_build_failure_merges_stderr_into_output() -> Result<()> {
    ...
    let result = client
        .call_tool(CallToolRequestParam {
            name: "build".into(),
            arguments: Some(object!({"working_directory": project_path.to_str().unwrap()})),
        })
        .await?;
    let text = format!("{:?}", result.content);
    assert!(
        text.contains("- Build failed"),
        "Expected build to fail: {text}"
    );
    assert!(
        text.contains("Error:"),
        "Should contain Error section with stderr"
    );
    ...
    assert!(
        after_output.contains("expected") || after_output.contains("error"),
        "Merged Output section missing stderr content: {text}"
    );
